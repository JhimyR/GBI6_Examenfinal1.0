...
```{r data}
library(readr)
df <- read_csv("C:/Users/HP/Documents/GBI6_Examenfinal1.0/data/mRNA_expr.csv")
...
# Nombraremos a la función de nombre long_df
long_df <- function(df) {
...
# Se instala la libreria tidyverse
library(tidyverse)

# Nombramos la funcion df_long <- long_df
df_long <- long_df(df)

Analisis <- df_long %>% group_by(gen)
...
cancer_mama <- Analisis %>% group_by(gen) %>% mutate(row = row_number()) %>%
  tidyr::pivot_wider(names_from = gen, values_from = expresion_level) %>%
  select(-row)

descr(cancer_mama) %>% tab_df(title = "Descripción estadística del cáncer de mama y su nivel de expresion de los diferentes genes",
                              file = "C:/Users/HP/Documents/GBI6_Examenfinal1.0/results/mRNA_expr_summary.doc") 
head(cancer_mama)
...
# En base a la resolucion de los Ejercicios 1.1 haste 1.5 se puede observar en la tabla
# de resumen de los diferentes tipos de enfermedades y su nivel de expresion segun sus genes.
# Además, de los cuales podemos contemplar:
# BRCA -> Carcionoma invasivo de mama
# KIPAN -> Cohorte Pan-riñón 
# KIRP -> Atlas del Genoma del Cáncer Carcinoma renal de células papilares
# LUSC -> Carcinoma de células escamosas de pulmón
# Existe la variancion de los datos, en funcion de los promedio, las obervaciones, tambien
# las diferentes variasbles en cada tabla.
...
ggsave("C:/Users/HP/Documents/GBI6_Examenfinal1.0/results/boxplot3.png", width = 6, height = 8, dpi = 300)
...
graficos_cancer[[5]][[2]]
ggsave("C:/Users/HP/Documents/GBI6_Examenfinal1.0/results/regresion11.pdf", width = 5, height = 10, dpi=300)
...
# Segun los resultados obtenido en al figura del boxplot3.png se observó una distribucion con valores # a los normales. Además, resultaods negativos o puntos en GATA3, con diferencias en los quintiles.
# como seunda parte de la grafica de regresion lineal , se pudo observar los puntos de dispersión, y
# tambien los niveles de expresion de genes era muy diferenciada entre los canceres 6, pero con mas 
# frecuencias en los canceres. Se puede observar los niveles de expresión del gen PTEN (gen1) 
# aunmenta, en relación al gen GAPDH.
